...
#include "arch35/causal_conv1d_regbase.h"
 ...
 protected:
     ...
     __aicore__ inline void RunSeqFnRolling(int32_t start, int32_t len, int32_t channelStart, int32_t baseDim,
                                            int32_t dim);
     __aicore__ inline void RunSeqOutputRing(int32_t start, int32_t len, int32_t channelStart, int32_t baseDim,
                                             int32_t dim);
     __aicore__ inline void PrimeOutputRingFromHistory(int32_t baseDim);
 ...
 protected:
     ...
     TBuf<QuePosition::VECCALC> outRingBuf;  // FP32 output-accumulator ring
 ...
 __aicore__ inline void CAUSAL_CONV1D_CLASS::InitSharedBuffersAndEvents()
 {
     ...
     pipe.InitBuffer(outRingBuf, OUT_RING_SLOTS * MAX_BLOCK_DIM * sizeof(float));
     AllocEvents();
 }
                                                    ...
                                                    int32_t baseDim, int32_t dim)
 {
     if (IsFnRollingFastPathEnabled()) {
         if constexpr (kTemplateWidth == 4) {
             RunSeqOutputRing(start, len, channelStart, baseDim, dim);
         } else {
 ...
 template <CAUSAL_CONV1D_TEMPLATE_ARGS>
 __aicore__ inline void CAUSAL_CONV1D_CLASS::PrimeOutputRingFromHistory(int32_t baseDim)
 {
     // Width-4 only. Seed the first three output accumulators with the contributions
     // of the (width-1)=3 history tokens already in ring slots 0,1,2 (= x[-3],x[-2],x[-1]).
     // Mirrors RestoreFnLocalPartials's op sequence exactly (same FP32 rounding), but
     // writes oacc[0..2] instead of state0/1/2:
     //   oacc[0] = w0*x[-3] + w1*x[-2] + w2*x[-1]
     //   oacc[1] = w0*x[-2] + w1*x[-1]
     //   oacc[2] = w0*x[-1]
     auto cl = CalcBufLayout::FromCalcBuf(calcBuf);
     LocalTensor<float> &weightF = cl.weightF;
     LocalTensor<float> &currF = cl.currF;
     LocalTensor<T> ring = inBuf.Get<T>();
     LocalTensor<float> oacc = outRingBuf.Get<float>();
     constexpr int32_t ringStart = MAX_WIDTH - 4;  // 0
     constexpr int32_t w0Idx = MAX_WIDTH - 4;       // 0

     Cast(currF, ring[ringStart * MAX_BLOCK_DIM], RoundMode::CAST_NONE, baseDim);
     PipeBarrier<PIPE_V>();
     Mul(oacc[0 * MAX_BLOCK_DIM], currF, weightF[w0Idx * MAX_BLOCK_DIM], baseDim);
     PipeBarrier<PIPE_V>();

     Cast(currF, ring[(ringStart + 1) * MAX_BLOCK_DIM], RoundMode::CAST_NONE, baseDim);
     PipeBarrier<PIPE_V>();
     Mul(oacc[1 * MAX_BLOCK_DIM], currF, weightF[w0Idx * MAX_BLOCK_DIM], baseDim);
     PipeBarrier<PIPE_V>();
     MulAddDst(oacc[0 * MAX_BLOCK_DIM], currF, weightF[(w0Idx + 1) * MAX_BLOCK_DIM], baseDim);
     PipeBarrier<PIPE_V>();

     Cast(currF, ring[(ringStart + 2) * MAX_BLOCK_DIM], RoundMode::CAST_NONE, baseDim);
     PipeBarrier<PIPE_V>();
     Mul(oacc[2 * MAX_BLOCK_DIM], currF, weightF[w0Idx * MAX_BLOCK_DIM], baseDim);
     PipeBarrier<PIPE_V>();
     MulAddDst(oacc[1 * MAX_BLOCK_DIM], currF, weightF[(w0Idx + 1) * MAX_BLOCK_DIM], baseDim);
     PipeBarrier<PIPE_V>();
     MulAddDst(oacc[0 * MAX_BLOCK_DIM], currF, weightF[(w0Idx + 2) * MAX_BLOCK_DIM], baseDim);
     PipeBarrier<PIPE_V>();
 }

 template <CAUSAL_CONV1D_TEMPLATE_ARGS>
 __aicore__ inline void CAUSAL_CONV1D_CLASS::RunSeqOutputRing(int32_t start, int32_t len, int32_t channelStart,
                                                              int32_t baseDim, int32_t dim)
 {
     // "scatter" formulation. Each input token x[t] is cast ONCE and
     // multiply-accumulated into the 4 in-flight output accumulators it touches
     // (outputs t..t+3) - all distinct ring slots, so the 4 MulAccs carry no
     // mutual RAW (vs main's rolling, which serialises a state0->state1->state2
     // shift chain and re-casts the input a 2nd time). ~7 V ops/token vs ~10.
     if constexpr (kTemplateWidth != 4) {
         return;
     }
     ...
     auto cl = CalcBufLayout::FromCalcBuf(calcBuf);
     LocalTensor<float> &weightF = cl.weightF;
     LocalTensor<float> &currF = cl.currF;   // FP32 cast of the current input token
     LocalTensor<float> &siluF = cl.accF;    // activation scratch (accF is unused in this path)
     LocalTensor<T> ring = inBuf.Get<T>();
     LocalTensor<float> oacc = outRingBuf.Get<float>();
     LocalTensor<T> outT = outBuf.Get<T>();
     const bool hasActivation = HasActivation();

     PrimeOutputRingFromHistory(baseDim);
     ...
     for (int32_t t = 0; t < len; ++t) {
         ...
         // Cast current input token once.
         Cast(currF, ring[slotCurr * MAX_BLOCK_DIM], RoundMode::CAST_NONE, baseDim);
         PipeBarrier<PIPE_V>();

         // Scatter x[t] into outputs t, t+1, t+2, t+3 (distinct accumulator slots).
         const int32_t so0 = OutRingSlot(t);
         MulAddDst(oacc[so0 * MAX_BLOCK_DIM], currF, weightF[3 * MAX_BLOCK_DIM], baseDim);  // completes y[t]
         if (t + 1 < len) {
             const int32_t so1 = OutRingSlot(t + 1);
             MulAddDst(oacc[so1 * MAX_BLOCK_DIM], currF, weightF[2 * MAX_BLOCK_DIM], baseDim);
         }
         if (t + 2 < len) {
             const int32_t so2 = OutRingSlot(t + 2);
             MulAddDst(oacc[so2 * MAX_BLOCK_DIM], currF, weightF[1 * MAX_BLOCK_DIM], baseDim);
         }
         if (t + 3 < len) {
             const int32_t so3 = OutRingSlot(t + 3);
             Mul(oacc[so3 * MAX_BLOCK_DIM], currF, weightF[0 * MAX_BLOCK_DIM], baseDim);  // inits y[t+3]
         }
         PipeBarrier<PIPE_V>();
         ...
         LocalTensor<T> outSlotT = outT[outSlot * MAX_BLOCK_DIM];
         if (hasActivation) {
             Silu(siluF, oacc[so0 * MAX_BLOCK_DIM], baseDim);
             PipeBarrier<PIPE_V>();
         }
         if (t >= 2) {
             WaitFlag<HardEvent::MTE3_V>(outMte3ToVEvent_[outSlot]);
         ...
         if constexpr (IsSameType<T, float>::value) {
             if (hasActivation) {
                 DataCopy(outSlotT, siluF, baseDim);
             } else {
                 DataCopy(outSlotT, oacc[so0 * MAX_BLOCK_DIM], baseDim);
             }
         } else {
             if (hasActivation) {
                 Cast(outSlotT, siluF, RoundMode::CAST_RINT, baseDim);
             } else {
                 Cast(outSlotT, oacc[so0 * MAX_BLOCK_DIM], RoundMode::CAST_RINT, baseDim);
             }
         }
         SetFlag<HardEvent::V_MTE3>(outVToMte3Event_[outSlot]);
